...
namespace RVO {
	...
	size_t RVOSimulator::addAgent(const Vector3 &position, const Vector3 &velocity)
	{
		if (defaultAgent_ == NULL) {
			return RVO_ERROR;
		}
		...
		agent->velocity_ = velocity;//Here changed, edited on 2016/11/9 by zzf
	...
	void RVOSimulator::doStep(size_t agent_id)
	{
...
#pragma omp parallel for
		...
		//for (int i = 0; i < static_cast<int>(agents_.size()); ++i) {
			agents_[agent_id]->computeNeighbors();
			agents_[agent_id]->computeNewVelocity();
		//}
...
#pragma omp parallel for
		...
		//for (int i = 0; i < static_cast<int>(agents_.size()); ++i) {
			//agents_[i]->update();
		//}
			agents_[agent_id]->velocity_ = (agents_[agent_id]->newVelocity_);//更新速度，仿真时可以注释掉
			agents_[agent_id]->position_ += agents_[agent_id]->velocity_ * (agents_[agent_id]->sim_)->timeStep_;
